...
def compare_embedding(embedding, facebank):
    ...
    dist = cos_sim(embedding, facebank)
    # dist = np.sum(np.square(diff), axis=1)
    min_idx = np.argmax(dist)
    print("相似度0.72~1计数："+str(np.sum(np.where( dist >= 0.65, 1, 0))))
    print("相似度0.5~0.72计数：" + str(np.sum(np.where((dist >= 0.5 ) & ( dist < 0.65), 1, 0))))
    print("相似度0~0.5计数：" + str(np.sum(np.where(dist < 0.5, 1, 0))))
    return min_idx,dist[min_idx]

def cos_sim(vector_a, vector_b):
    a = []
    for i in vector_b:
        sim = cos_sim_single(vector_a, i)
        a.append(sim)
    return np.array(a)


def cos_sim_single(vector_a, vector_b):
    """
    计算两个向量之间的余弦相似度
    :param vector_a: 向量 a
    :param vector_b: 向量 b
